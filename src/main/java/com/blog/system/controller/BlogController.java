...
public class BlogController {
    ...
    public String newBlog(@RequestHeader("token") String token,@RequestBody Blog blog) {
        ...
        blog.setAuthor(userMapper.findByUser_name(user_name).getId());
    ...
    @GetMapping(value = "/get/{id}",produces = "text/plain;charset=UTF-8")
    ...
    @GetMapping(value = "/get/personal",produces = "text/plain;charset=UTF-8")
    ...
    @GetMapping(value = "/get/myBlogs",produces = "text/plain;charset=UTF-8")
    ...
    //计数当前用户的博客数量
    @GetMapping("/count/myBlogs")
    public String getPersonal(@RequestHeader("token") String token) {
        if(token==null||jwtUtil.parseJWT(token)==null) {
            SendStringDTO sendStringDTO=new SendStringDTO();
            sendStringDTO.setCode(false);
            sendStringDTO.setMessage("User not logged in");
        ...
        SendCountDTO sendCountDTO = new SendCountDTO();
        sendCountDTO.setCode(true);
        sendCountDTO.setCnt(blogMapper.countByAuthorID(userMapper.findByUser_name(user_name).getId()));
        sendCountDTO.setPage((sendCountDTO.getCnt() + 9) / 10);
        return JSON.toJSONString(sendCountDTO);
    }
